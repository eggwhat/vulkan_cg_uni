#include "marble_render_system.hpp"
...
namespace vcu {
	...
	MarbleRenderSystem::MarbleRenderSystem(VcuDevice& device, VkRenderPass renderPass, VkDescriptorSetLayout globalSetLayout,
	...
	MarbleRenderSystem::~MarbleRenderSystem() {
		vkDestroyPipelineLayout(vcuDevice.device(), pipelineLayout, nullptr);
	}

	void MarbleRenderSystem::createPipelineLayout(VkDescriptorSetLayout globalSetLayout) {
	...
	void MarbleRenderSystem::createPipeline(VkRenderPass renderPass, const std::string& vertexShaderFile, const std::string& fragmentShaderFile) {
	...
	void MarbleRenderSystem::renderGameObjects(FrameInfo &frameInfo) {
		...
		for (auto& kv : frameInfo.gameObjects) {
			auto& obj = kv.second;
			if (obj.model == nullptr || obj.type != 5) continue;
