#include "no_txt_render_system.hpp"
...
namespace vcu {
	...
	NoTxtRenderSystem::NoTxtRenderSystem(VcuDevice& device, VkRenderPass renderPass, VkDescriptorSetLayout globalSetLayout,
	...
	NoTxtRenderSystem::~NoTxtRenderSystem() {
		vkDestroyPipelineLayout(vcuDevice.device(), pipelineLayout, nullptr);
	}

	void NoTxtRenderSystem::createPipelineLayout(VkDescriptorSetLayout globalSetLayout) {
	...
	void NoTxtRenderSystem::createPipeline(VkRenderPass renderPass, const std::string& vertexShaderFile, const std::string& fragmentShaderFile) {
	...
	void NoTxtRenderSystem::renderGameObjects(FrameInfo &frameInfo) {
		...
		for (auto& kv : frameInfo.gameObjects) {
			auto& obj = kv.second;
			if (obj.model == nullptr || obj.type != 3) continue;
